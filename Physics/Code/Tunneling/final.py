...
def doublewell(x):
    ...
    if(x>=a_V1)and(x<=b_V1):
        potential1=V01+1
    elif((x>=a_V2)and(x<=b_V2)):
        potential1=V01+1
    elif((x>b_V1)and(x<a_V2)):
        potential1=Vmiddle+1
    else:
        potential1=0.0+1
    return potential1
...
def expectedPosition(name): # plot where the expected positions are
    ...
    if name=='4':
        name='width5'
    plt.plot(position[:,0], position[:,1],label='expected position '+name)
    ...
    plt.legend()
    return None
names = ['width2_5','4','width7_5']
for name in names:
    expectedPosition(name)
#expectedPosition('oscillator')
#expectedPosition('well2')
plt.show()
...
def expectedMomentum(name): #  plot where the expected momentum is 
    ...
    if name=='4':
        name='width5'
    plt.plot(momentum[:,0], momentum[:,1]*2,label='expected momentum '+name)
    plt.xlabel('Time t [a.u.]')
    ...
    return None
for name in names:
    expectedMomentum(name)
#expectedMomentum('well2')
#expectedMomentum('oscillator')
plt.show()
...
def coordinate_parts(file_name, y_shift,V,s=0): #animate the real and imaginary parts of space wavee function
    ...
    plt.show()
    return None
#coordinate_parts('oscillator',2.5,4,0)
...
def coordinate_density(file_name, y_shift,V,s=0): #plot the density of coordinate space
    ...
    if s!=0:
       # ax.set_ylim(2, 3)
        ...
        ax.set_ylim(2, 3)
        ani.save('/Users/phihung/NumMethod/first/final/dx_potential_zoomed'+file_name+'.gif', writer='pillow')
    plt.show()
    return None
#coordinate_density('oscillator',2.5,4,0)
