...
double precision window,x,k
double complex phi !Fourier transform is complex
double complex tmp
integer n,m

!fftpack
double precision wsave(4*NN+15)
double complex f_fft(NN)


!<--- first we do simple Fourier
open(file='simple_ReIm_window_201points.dat',unit=11)
open(file='simple_abs_window_201points.dat',unit=12)
do m=0,NN-1 !k
  k=k0+dk*m
  tmp=(0.0,0.0)
  do n=0,NN-1
    x=x0+dx*n
    tmp=tmp+window(x)*exp(-2.0*pi*Im*(m-a)*(n-a)/NN)
  enddo
  phi=tmp!*dk/sqrt(2.0*pi)
...
open(file='dft_ReIm_window_201points.dat',unit=11)
open(file='dft_abs_window_201points.dat',unit=12)
...
do m=1,NN !k
  ...
  if(m<=NN/2)then
      ...
      k=-2.0*pi*(NN+1-m)/(dx*NN)
  endif

  write(11,*) k,dreal(f_fft(m)),aimag(f_fft(m)) !real and imaginary parts
  write(12,*) k,abs(f_fft(m))**2       !|phi|^2
enddo
close(unit=11)
close(unit=12)
end

double precision function window(x)
  double precision x
