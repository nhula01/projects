subroutine duffing(B) 
   ...
   open(file='/Users/phihung/NumMethod/first/homework/hw_06/duffing_beta_phase14.dat',unit=11)
      t=t_min
      x1=x0
      x2=v0
      NN=0 !count number of total time steps for FFT
      do while(t<=t_max)
         ...
         tmp(2)=x2
         k1(1)=tmp(2)
         k1(2)=-tmp(1)**3-.1*tmp(2)+B*cos(t)
         !second RK step
         tmp(1)=x1+0.5*k1(1)*dt
         tmp(2)=x2+0.5*k1(2)*dt
         k2(1)=tmp(2)
         k2(2)=-tmp(1)**3-.1*tmp(2)+B*cos(t)
         !third RK step
         tmp(1)=x1+0.5*k2(1)*dt
         tmp(2)=x2+0.5*k2(2)*dt
         k3(1)=tmp(2)
         k3(2)=-tmp(1)**3-.1*tmp(2)+B*cos(t)
         !fourth RK step
         tmp(1)=x1+k3(1)*dt
         tmp(2)=x2+k3(2)*dt
         k4(1)=tmp(2)
         k4(2)=-tmp(1)**3-.1*tmp(2)+B*cos(t)
   ...
   open(file='/Users/phihung/NumMethod/first/homework/hw_06/duffing_FFT14.dat',unit=12)
      do n=1,NN/2 !only interested in positive frequencies
            omega=1.0*float(n-1)/(dt*NN) !omega here in Hz
            write(12,*) omega,abs(f_fft(n))
      enddo
   close(12)
   close(11)
end subroutine duffing
