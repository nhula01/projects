subroutine random_duffing(B)
   implicit none
   double precision, intent(in) :: B
   double precision, parameter :: pi=3.141592653589793
   double precision eps  !Van der Pol parameter - we will generate it randomly
   integer, parameter :: n_random=1 !generate random number every n_random steps
   double precision temp
   double precision, parameter :: t_min=0.0d0,t_max=300 !interval of integration of ODE
   double precision, parameter :: x0=0.1 !initial condition x(0)=0.5
   double precision, parameter :: v0=0.0 !initial condition v(0)=0.0
   double precision, parameter :: dt=3.0d-2               !integration step, we define it below
   integer, parameter :: N_steps=200 !number of steps per period
   ...
   open(file='/Users/phihung/NumMethod/first/homework/hw_06/duffing_beta_random_phase_1_14.dat',unit=12)

      call random_number(temp) !temp is a random number between 0 and 1
      f=2.0*(temp-.5) !eps is Van der Pol parameter - between 0 and 1
         !but we can scale it to any interval
      ...
      do while(t<=t_max)
         NN=NN+1
         if(mod(NN,n_random)==0)then !every n_random steps generate random number
            call random_number(temp) !tmp is a random number between 0 and 1
            f=2.0*(temp-.5)
         endif
         ...
         tmp(2)=x2
         k1(1)=tmp(2)
         k1(2)=-tmp(1)**3-.1*tmp(2)+B*f

         !second RK step
         tmp(1)=x1+0.5*k1(1)*dt
         tmp(2)=x2+0.5*k1(2)*dt
         k2(1)=tmp(2)
         k2(2)=-tmp(1)**3-.1*tmp(2)+B*f

         !third RK step
         tmp(1)=x1+0.5*k2(1)*dt
         tmp(2)=x2+0.5*k2(2)*dt
         k3(1)=tmp(2)
         k3(2)=-tmp(1)**3-.1*tmp(2)+B*f

         !fourth RK step
         tmp(1)=x1+k3(1)*dt
         tmp(2)=x2+k3(2)*dt
         k4(1)=tmp(2)
         k4(2)=-tmp(1)**3-.1*tmp(2)+B*f
      ...
      open(file='/Users/phihung/NumMethod/first/homework/hw_06/duffing_random_frequency_1_14.dat',unit=11)
      do n=1,NN/2 !only interested in positive frequencies
         omega=1.0*float(n-1)/(dt*NN) !omega here in Hz
         write(11,*) omega,abs(f_fft(n))
      enddo
      close(unit=11)
   close(unit=12)
endsubroutine random_duffing

program set3
   implicit none
   double precision :: B = 14
   call random_duffing(B)
end program set3
