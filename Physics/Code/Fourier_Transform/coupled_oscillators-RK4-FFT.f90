...
double precision, parameter :: m=2.0,kb=100.0 !mass, spring constant
...
double precision period,frequency
...
period=2.0*pi*sqrt(m/kb) !we use largest k to estimate the smallest period
frequency=1.0/period
write(*,*) 'frequency of oscillator #1/#2 in Hz = ',frequency
dt=period/N_steps
  ...
  do while(t<=t_max)
   !<~~~ first RK step
    ...
    tmp(4)=v2
     ...
     rk1(2)=-(kb/m)*tmp(1)+ks*(tmp(3)-tmp(1))-mu_k1*g*temp
     rk1(3)=tmp(4)
                    temp=sign(1.0d0,tmp(4))
     rk1(4)=-(kb/m)*tmp(3)-ks*(tmp(3)-tmp(1))-mu_k2*g*temp
    ...
    tmp(4)=v2+0.5*rk1(4)*dt
     rk2(1)=tmp(2)
                   temp=sign(1.0d0,tmp(2))
     rk2(2)=-(kb/m)*tmp(1)+ks*(tmp(3)-tmp(1))-mu_k1*g*temp
     rk2(3)=tmp(4)
                   temp=sign(1.0d0,tmp(4))
     rk2(4)=-(kb/m)*tmp(3)-ks*(tmp(3)-tmp(1))-mu_k2*g*temp
    ...
    tmp(4)=v2+0.5*rk2(4)*dt
     rk3(1)=tmp(2)
                   temp=sign(1.0d0,tmp(2))
     rk3(2)=-(kb/m)*tmp(1)+ks*(tmp(3)-tmp(1))-mu_k1*g*temp
     rk3(3)=tmp(4)
                   temp=sign(1.0d0,tmp(4))
     rk3(4)=-(kb/m)*tmp(3)-ks*(tmp(3)-tmp(1))-mu_k2*g*temp
    ...
    tmp(4)=v2+rk3(4)*dt
     rk4(1)=tmp(2)
                   temp=sign(1.0d0,tmp(2))
     rk4(2)=-(kb/m)*tmp(1)+ks*(tmp(3)-tmp(1))-mu_k1*g*temp
     rk4(3)=tmp(4)
                   temp=sign(1.0d0,tmp(4))
     rk4(4)=-(kb/m)*tmp(3)-ks*(tmp(3)-tmp(1))-mu_k2*g*temp
...
open(file='coupled_oscillators_5_FFT.dat',unit=11)
do n=1,NN/2 !only interested in positive frequencies
  omega=1.0*float(n-1)/(dt*NN) !omega here in Hz
  if(omega.ne.0.0.and.omega<=10.0*frequency)then !recording only frequencies below 10*frequency of 1 oscillator
