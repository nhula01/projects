subroutine goodpart(distance1, eps_dielectric, name)
    implicit none
    ...
    double precision, parameter :: dt_eps0=dt/eps0,dt_mu0=dt/mu0 !constants for Maxwell's eq.
    !~~~======== spatial position of the source =========~~~!
    integer, parameter :: k_source=50 !placing source 50 nm to the right of z0
    ...
    double precision :: distance!=600.0345098d-9 !distance between mirrors
    double precision :: distance1!=600.0345098d-9 !distance between mirrors
    ...
    !
    !~~~~======================== dielectric in-between mirrors ========================~~~~!
    double precision :: eps_dielectric!=1.5*1.5 !permittivity of medium (1.5*1.5 for glass, 1.0*1.0 for vacuum)
                                                        !epsilon=(refractive index)**2
    double precision :: dt_eps_r!=dt/(eps0*eps_dielectric) !constant for Maxwell's eq.
    logical FB_dielectric(Nz) !logical array defining dielectric on FDTD grid
    !
    !~~~~~~~ Fourier analysis !!!!!!!
    double precision, parameter :: omega_min=0.01,omega_max=4.0
    integer N_w
    ...
    distance=distance1*1.0d-9
    dt_eps_r=dt/(eps0*eps_dielectric)!dt_eps_r!=dt/(eps0*eps_dielectric)
    !<--- grid
    do k=1,Nz
        z(k)=z0+dz*(k-1)
    enddo
    ...
    if((z(k)>-(distance/2.0)).and.(z(k)<(distance/2.0)).and.(.not.(FB_metal(k))))then !dielectric between mirrors
        FB_dielectric(k)=.true.
    endif
    enddo
    ...
    do k=2,Nz-1
        ...
        Ex(k)=tmpE
        elseif(FB_dielectric(k))then
        Ex(k)=Ex(k)+dt_eps_r*(Hy(k-1)-Hy(k))/dz
        else
...
end subroutine goodpart

subroutine goodpart2(distance1, eps_dielectric)
    implicit none
    character(len=100) :: file_name
    !~~~===== fundamental constants =====~~~!
    double precision, parameter :: pi=3.141592653589793,c=299792458.0
    ...
    !~~~= other parameters and variables =~~~!
    integer :: i,nt_step,nt_tmp, nt_psi, t_temp
    double precision :: detection(Nt) 
    ...
    !  PML for right side Ex
        ...
        detection(n)=Ex(Nz-npml-50)!detecting Ex 500 nm to the left of zM
    !--------------------------------------------------------------------------!
    !~~~~~~~~~~~~~~~~~~~~~~~~         detection          ~~~~~~~~~~~~~~~~~~~~~~!
    !--------------------------------------------------------------------------!
        if(mod(n,Nt_skip)==0)then
    ...
    open(file='/Users/phihung/NumMethod/first/homework/hw_09/electric_t_2_glass.dat',unit=32)
        do i=1,Nz
            write(32,*) (Ext(i,n),n=1,nt_psi) !here time in fs
        enddo
    close(unit=32)

    open(file='/Users/phihung/NumMethod/first/homework/hw_09/magnetic_t_2_glass.dat',unit=32)
        do i=1,Nz-1
            write(32,*) (Hyt(i,n),n=1,nt_psi) !here time in fs
        enddo
    close(unit=32)

    open(file='/Users/phihung/NumMethod/first/homework/hw_09/detection2_glass.dat',unit=32)
        do n=1,Nt
            if(mod(n,n_save)==0)then
        ...
        enddo
    close(unit=32)
end subroutine goodpart2

program main
    implicit none
    character(len=3) :: name='3_5'
    double precision :: distance=600, eps_dielectric=1.5*1.5
    integer :: i
    character(len=5) :: str
    character(len=8) ::new_str
    !do i=300,1000,100
     !   distance = i 
      !  write(str, '(I5.5)') i ! convert to string
      !  new_str = str//name
      !  call goodpart(distance,eps_dielectric,new_str)
    !enddo 
    call goodpart2(distance,eps_dielectric)
end program main
!
