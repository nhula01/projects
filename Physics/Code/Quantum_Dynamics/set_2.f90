program set2
    implicit none
    integer :: type, i
    character(len=100) :: name='oscillate0_5.dat'
    ...
    !now explore ground
    type=1 !ground
    call timeprop(name,type)
    type=2 !excited
    call timeprop(name,type)
end program set2

subroutine timeprop(name,choice)
    implicit none
    ...
    integer, parameter :: n_save=250 !save wavefunction every n_save steps
    ...
    open(file='/Users/phihung/NumMethod/first/homework/hw_08/p2states12.dat',unit=11)
        do i=1,N
    ...
    !<--- since V depends on t we cannot precalculate exp(V)
    ...
    do while(nt<=nt_final)
        ...
        !--- kinetic part ---! root n for each transform -> n for 2 transformations
        call zfftf(N,psi,wsave) !forward Fourier transform to momentum space
        psi=exp_T*psi/float(N) !1/N is needed due to forward and backward fft
        call zfftb(N,psi,wsave) !backward Fourier transform to coordinate space

        !--- potential part ---!
        do i=1,N
            x=a+dx*float(i)
            exp_V=exp(-Im*dpotential(x,t)*dt/hbar)
            psi(i)=exp_V*psi(i) !update the second part
        enddo
        ...
        if(mod(nt,n_save)==0)then !<-- save psi every n_save steps
            do i=i_min,i_max
                psi_save(i,nt/n_save)=abs(psi(i))**2 !save absolute value of position in different t values
            enddo
        endif
        ...
        delta_x(nt)=sqrt(abs(tmp2)-av_x(nt)**2) !uncertainty of x
    ...
    filename = '/Users/phihung/NumMethod/first/homework/hw_08/p2x_vs_t-state1' // name
    ...
    filename = '/Users/phihung/NumMethod/first/homework/hw_08/p2psi_xt-state1' // name
    open(file=filename,unit=11)
        do i=i_min,i_max
            write(11,*) (psi_save(i,nt),nt=2,nt_final/n_save) !wite the state at i evolving through all t in a row
        enddo
    close(unit=11)
...
subroutine generateDVR
    ...
    double precision T,V,potentialDVR
    ...
    open(file='/Users/phihung/NumMethod/first/homework/hw_08/p2energies.dat',unit=11)
...
double precision function dpotential(x,t)
    implicit none
    double precision, parameter :: pi=3.141592653589793
    double precision, parameter :: omega=1.0 !all in atomic units
    double precision, parameter :: mass=5.0 !1 in atomic units means it is electron
    double precision, parameter :: omega_osc=0.5
    double precision, parameter :: A_osc=1.0 !amplitude of oscillations
    double precision x,t
    if(x<=0)then
        dpotential= 0.5*mass*(omega*x)**2
    else
        dpotential=0.5*mass*(omega*x)**2*(1+A_osc*sin(omega_osc*t))
    endif
end function dpotential

double precision function potentialDVR(x)
    implicit none
    double precision, parameter :: omega=1 !all in atomic units
    double precision, parameter :: mass=5.0 !1 in atomic units means it is electron
    double precision x
    potentialDVR=0.5*mass*(omega*x)**2
end function potentialDVR
